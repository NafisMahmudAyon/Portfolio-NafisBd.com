...
const CustomMobileLink = ({ href, title, className="", toggle,}) => {
    ...
    return (
        <button href={href} className={`${className} relative group my-2 text-light dark:text-dark`} onClick={handleClick} >
            ...
            <span className={`h-[2px] inline-block group-hover:w-full transition-[width] ease duration-300 bg-light dark:bg-dark absolute left-0 -bottom-0.5 
            ${router.asPath === href ? 'w-full' : 'w-0'} dark:bg-light
            `}>&nbsp;</span>
        </button>
    )
    ...
    <header className='w-full px-32 py-8 font-medium flex items-center justify-between relative dark:text-light z-10 lg:px-16 md:px-12 sm:px-8'>
        ...
        {/* mobile menu */}
        {
            ...
            <nav className='flex items-center flex-col justify-center'>
                <CustomMobileLink href='/' title="Home" className=' uppercase font-bold ' toggle={handleClick} />
                <CustomMobileLink href='/about' title="About" className=' uppercase font-bold' toggle={handleClick} />
                <CustomMobileLink href='/projects' title="Projects" className=' uppercase font-bold' toggle={handleClick} />
                <CustomMobileLink href='/articles' title="Articles" className=' uppercase font-bold' toggle={handleClick} />
            </nav>
            
            <nav className='flex items-center justify-center flex-wrap mt-2'>
                <motion.a href="https://www.facebook.com/nafismahmud0/" target={"_blank"} whileHover={{ y:-2 }} whileTap={{ scale:0.9 }} className='w-6 mr-3 sm:mx-1 fill-light dark:fill-dark'><FacebookIcon /></motion.a>
                <motion.a href="https://github.com/NafisMahmudAyon" target={"_blank"} whileHover={{ y:-2 }} whileTap={{ scale:0.9 }} className='w-6 mx-3 sm:mx-1 fill-light dark:fill-dark'><GithubIcon /></motion.a>
                <motion.a href="https://www.linkedin.com/in/nafis-mahmud-ayon-760b45190/" target={"_blank"} whileHover={{ y:-2 }} whileTap={{ scale:0.9 }} className='w-6 mx-3 sm:mx-1 fill-light dark:fill-dark'><LinkedInIcon /></motion.a>
                <motion.a href="mailto:nafismahmudayon@gmail.com" target={"_blank"} whileHover={{ y:-2 }} whileTap={{ scale:0.9 }} className='w-6 mx-3 sm:mx-1 fill-light dark:fill-dark'><WhatsAppIcon /></motion.a>
                <motion.a href="mailto:nafismahmudayon@gmail.com" target={"_blank"} whileHover={{ y:-2 }} whileTap={{ scale:0.9 }} className='w-6 mx-3 sm:mx-1 fill-light dark:fill-dark'><TelegramIcon /></motion.a>
                <motion.a href="mailto:nafismahmudayon@gmail.com" target={"_blank"} whileHover={{ y:-2 }} whileTap={{ scale:0.9 }} className='w-6 ml-3 sm:mx-1 fill-light dark:fill-dark'><GmailIcon /></motion.a>
                <button
                    onClick={() => setMode(mode === "light" ? "dark" : "light")}
                    className={`ml-4 sm:ml-3 flex items-center justify-center rounded-full p-1 ${mode === "light" ? "bg-dark text-light" : "bg-light text-dark"}`}
                >
                    {
                        mode === "dark" ?
                        <SunIcon className={"fill-dark"} />
